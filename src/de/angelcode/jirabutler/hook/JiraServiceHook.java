...
/**
 * This class parses the github HTTP-POST githubRequest for relevant payload information like the username of the pusher, the commit gitCommitMessage and so on...
 * @author bennyn
 */
public final class JiraServiceHook
{
  ...
  /**
   * Init-constructor which awaits the complete HTTP githubRequest from github.
   ...
   */
  public void convertGithubRequestToJson(String githubRequest) throws JiraButlerException
  {
    this.githubRequest = githubRequest;

    // Convert the payload from the github githubRequest into a JSON object
    int payloadStart = this.githubRequest.indexOf("payload=");
    String payloadAscii = this.githubRequest.substring(payloadStart + 8, this.githubRequest.length());
  ...
  /**
   * Parses the name of the current Git branch (in which the changes are pushed) and uses it as JIRA project jiraProjectVersion.
  ...
  private void parseVersion()
  {
    ...
    while (this.jiraProjectVersion.contains("/"))
    {
      ...
      this.jiraProjectVersion = this.jiraProjectVersion.substring(positionSlash + 1, this.jiraProjectVersion.length());
    }
  }

  /**
   * Parses the Git commit gitCommitMessage for the JIRA issue key. It expects an '@' as delimtter.
   * For example: If the commit gitCommitMessage is: 'SWQ-11@My commit gitCommitMessage', then this method
   * extracts the character sequence in front of the '@' and takes it for the JIRA issue key.
   * The character sequence which follows the '@' is the commit gitCommitMessage. In our example this
   * would be 'My commit gitCommitMessage' and the key will be 'SWQ-11'.
   *
   * = Short example =
   * Git-Commit gitCommitMessage: SWQ-11@My commit gitCommitMessage
   * JIRA issue key: SWQ-11
   * Message for the JIRA comment: My commit gitCommitMessage
  ...
  private void parseMessage()
  {
    ...
    if (this.gitCommitMessage.contains("@"))
    {
      ...
      this.gitCommitMessage = gitCommitMessage.substring(positionAt + 1, gitCommitMessage.length());
    }
  }
